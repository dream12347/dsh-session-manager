window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		//#endregion
		//#region src/client/menu-injection.ts
		/**
		...
		*/
		function isSessionMenu(itemTexts, forkText) {
		...
		*/
		function findSessionByTitle(snapshot, title) {
			for (const id of snapshot.ids) {
				...
				if (summary !== void 0 && !summary.blank && summary.displayTitle === title) return summary;
			}
		}
		//#endregion
...
`;
		...
		*/
		async function deleteFromMenu(session, title, sessions) {
		...
		function apply(ctx) {
			...
			ctx.effect(() => {
				...
				let frame = 0;
				const injectDelete = (menu) => {
					if (injectedMenu === menu || pendingTitle === null) return;
					const session = findSessionByTitle(sessions.list.getSnapshot(), pendingTitle);
					if (session === void 0) return;
					if (!isSessionMenu([...menu.querySelectorAll("[role=\"menuitem\"]")].map((item) => item.textContent ?? ""), isZh() ? "分叉会话" : "Fork session")) return;
					...
					item.addEventListener("click", (event) => {
						...
						deleteFromMenu(session, pendingTitle ?? session.displayTitle, sessions);
					});
					(menu.firstElementChild ?? menu).appendChild(item);
					injectedMenu = menu;
				};
				const schedule = () => {
					if (frame !== 0) return;
					frame = window.requestAnimationFrame(() => {
						frame = 0;
						for (const menu of document.querySelectorAll("body > [role=\"menu\"]")) injectDelete(menu);
					});
				};
				const observer = new MutationObserver(schedule);
				observer.observe(document.body, {
				...
				const onDocClick = (event) => {
					const target = event.target;
					if (!(target instanceof Element)) return;
					const button = target.closest("[role=\"treeitem\"] button[aria-label]");
					...
					for (const span of row.querySelectorAll("span")) {
						...
						if (text !== "" && findSessionByTitle(sessions.list.getSnapshot(), text) !== void 0) {
							pendingTitle = text;
							break;
						}
					}
		...
		*/
		...
		exports.NS = NS;
		exports.apply = apply;
		exports.findSessionByTitle = findSessionByTitle;
		exports.inject = inject;
		exports.isSessionMenu = isSessionMenu;
		exports.name = name;
